...
(() => {
  ...
  const utils = {
    ...
    getFirstFocusableEl(el) {
      el = !el ? document : el;
      const focusableSelectors = 'input, select, textarea, button:not([disabled]), [href]:not(use), [tabindex]:not([tabindex="-1"])';
    ...
    setFocusFirstFocusableEl(el) {
      const focusableEl = utils.getFirstFocusableEl(el);
...
(() => {
  ...
  class Icon {
    ...
    /**
     * Description...
     * @param {string} src (optional) param description...
     * @private
     */
    loadSvgSprite(src) {
  ...
  /**
   * a common icon module used as window.sg.common.icon
   * @module window/sg/common/icon
   */
  const icon = {
    /**
     * Description...
     ...
     * Description...
     * @func setSvgs
     * @param {HTMLElement} el=document (optional) param description...
     * @static
     */
    setSvgs(el = document) {
      if (Icon.instance) {
  ...
  //   },
  //   // element 내 svg icon 을 다시 그리기 위해 제거 후 재 삽입
  //   update(el = document) {
  //     const svgs = el.querySelectorAll('svg.icon');
  //
  //     svgs.forEach((el) => {
  ...
  //         parentNode.children[index].insertAdjacentHTML('afterend', html);
  //       } else {
...
(() => {
  ...
  const textField = {
    initAll() {
      [...document.querySelectorAll('.text-field-v2')].forEach((el) => {
        if (!TextField.instances.has(el)) {
          new TextField(el);
        }
      });
  ...
  class Menu  {
    constructor(el, index = -1) {
      ...
      this.handler = {
        ...
        selectEl: {
          ...
          click: () => {
            console.log('click', this.index, this.isOpen);
            if (getCurrentDevice() === 'mobile') {
              if (this.isOpen) {
                this.close();
              } else {
    ...
    setElements() {
      ...
      this.options.forEach((el) => {
        ...
        this.els.listEl.append(`<li class="menu__list-option-wrap" role="presentation"${tagsStr}><button class="menu__list-option" role="option"${titleStr}><span class="menu__list-option-text">${el.text}</span><svg class="menu__list-option-icon" focusable="false"><use xlink:href="#done-bold" href="#done-bold"></use></svg></button></li>`);
      });
      ...
      if (getCurrentDevice() === 'desktop') {
        if (this.isOpen) {
          ...
          if (this.openDir === 'up') {
            ...
            if (this.els.el.hasClass('menu--text-field')) {
              posY -= parseInt(this.els.el.css('padding-bottom'), 10);
            }
